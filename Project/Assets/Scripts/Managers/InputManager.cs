...
public class InputManager : MonoBehaviour
{
		...
		void Update ()
		{
				...
				if (Input.GetAxis ("Horizontal") < 0) {
						...
						if (player.OnGround) {
								if (currentAnim == player.AnimationGauche.Count - 1) {
										currentAnim = 0;
								} else {
										currentAnim ++;
								}
				...
				sr.sprite = player.AnimationSautGauche [currentAnim];
			}
				}
				if (Input.GetAxis ("Horizontal") > 0) {
						gameObject.transform.Translate (new Vector3 (player.Speed, 0, 0));
						if (player.OnGround) {
								if (currentAnim == player.AnimationDroite.Count - 1) {
										currentAnim = 0;
								} else {
										currentAnim ++;
								}
				...
				sr.sprite = player.AnimationSautDroite [currentAnim];
			}
